...
function createHtmlDocument(message, err, status) {
    console.log(err)
...
function finalhandler(ctx) {
    return function (err) {
        ...
        if (err) {
            ...
            // not found
            status = 404
            msg = 'Cannot ' + ctx.req.method + ' ' + ctx.req.url
        }
...
function send(ctx, status, message, err) {
    // response body
    var body = createHtmlDocument(message, err, status)
